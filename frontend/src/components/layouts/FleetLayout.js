...
export default function FleetLayout({ children }) {
  ...
  const roleColor = '#fb923c';

  const items = [
    { path:'/fleet',             label:'Fleet Dashboard', icon:'🚗' },
    { path:'/fleet/maintenance', label:'Maintenance',     icon:'🔧' },
    { path:'/fleet/routes',      label:'Route AI',        icon:'🗺' },
  ];

  return (
    <div style={{ display:'grid', gridTemplateColumns:'240px 1fr', minHeight:'100vh', fontFamily:'DM Sans,sans-serif' }}>
      <aside style={{ background:'#0f0900', borderRight:`1px solid ${roleColor}18`, display:'flex', flexDirection:'column' }}>
        <div style={{ padding:'1.75rem 1.5rem 1.25rem', borderBottom:`1px solid ${roleColor}15` }}>
          ...
          <div style={{ fontSize:'0.65rem', color:roleColor, letterSpacing:'0.2em', marginTop:'3px', fontWeight:'600' }}>FLEET MANAGER</div>
        </div>
        ...
        <div style={{ padding:'1rem 0.75rem', borderTop:`1px solid ${roleColor}15` }}>
          <div style={{ display:'flex', alignItems:'center', gap:'10px', padding:'0.6rem 0.9rem', borderRadius:'10px', background:`${roleColor}08`, marginBottom:'0.5rem' }}>
            ...
            <div>
              ...
              <div style={{ fontSize:'0.65rem', color:roleColor }}>FLEET MGR</div>
            </div>
          </div>
