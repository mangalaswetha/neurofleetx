...
export default function CustomerLayout({ children }) {
  ...
  const { getBookingsForCustomer } = useBooking();
  const userId = localStorage.getItem('userId');
  const name = localStorage.getItem('name') || 'Customer';
  const myRides = getBookingsForCustomer(userId);
  const active = myRides.filter(b => b.status==='ACCEPTED'||b.status==='IN_PROGRESS').length;
  const roleColor = '#c482ff';

  const items = [
    { path:'/customer',       label:'Home',      icon:'🏠' },
    { path:'/customer/book',  label:'Book Ride', icon:'🚗' },
    { path:'/customer/rides', label:'My Rides',  icon:'📋', badge: active },
  ];

  return (
    <div style={{ display:'grid', gridTemplateColumns:'240px 1fr', minHeight:'100vh', fontFamily:'DM Sans,sans-serif' }}>
      <aside style={{ background:'#09060f', borderRight:`1px solid ${roleColor}18`, display:'flex', flexDirection:'column' }}>
        <div style={{ padding:'1.75rem 1.5rem 1.25rem', borderBottom:`1px solid ${roleColor}15` }}>
          ...
          <div style={{ fontSize:'0.65rem', color:roleColor, letterSpacing:'0.2em', marginTop:'3px', fontWeight:'600' }}>CUSTOMER PORTAL</div>
        </div>
        ...
        <div style={{ padding:'1rem 0.75rem', borderTop:`1px solid ${roleColor}15` }}>
          <div style={{ display:'flex', alignItems:'center', gap:'10px', padding:'0.6rem 0.9rem', borderRadius:'10px', background:`${roleColor}08`, marginBottom:'0.5rem' }}>
            ...
            <div>
              ...
              <div style={{ fontSize:'0.65rem', color:roleColor }}>CUSTOMER</div>
            </div>
          </div>
